...
var map_data;
var map_visualizer;
window.map_data_callback = function (json) {
    map_data = json;
};
...
function fit_window_size() {
    var svg = document.getElementById('mainmap');
...
var drag_origin_x = 0;
var drag_origin_y = 0;
var map_offset_x = (window.innerWidth - 50) / 2;
var map_offset_y = (window.innerHeight - 180) / 2;
document.addEventListener('mouseup', function (e) {
    dragging = false;
});
document.addEventListener('mousemove', function (e) {
    if (dragging) {
        var move_x = e.clientX - drag_origin_x;
        var move_y = e.clientY - drag_origin_y;
...
var MapVisualizer = (function () {
    function MapVisualizer(map_data) {
        ...
        this.vertex_stations = {};
        this.line_edge_id_to_svg_obj = {};
        this.time_circle_svg_obj = {};
        this.station_id_to_bbox_size = {};
        this.center_station_id = 0;
        this.px_per_minute = 20;
        this.station_name_size = 15;
    ...
    MapVisualizer.prototype.create_svg_objects = function () {
        var _this = this;
        ...
        var bg = paper_root.rect(0, 0, 10000, 10000).attr({ 'fill': 'white' });
        ...
        var _loop_1 = function () {
            ...
            svg_station_name_minibox = paper.circle(1, 1, 10).attr({ fill: 'white', stroke: 'black', strokeWidth: '2px', title: station.station_name });
            this_1.station_id_to_svg_minibox_obj[String(station.station_id)] = svg_station_name_minibox;
            var _this_cap = this_1;
        ...
        var this_1 = this, station, svg_station_name_minibox;
        for (var i = 0; i < map_data.stations.length; i++) {
            _loop_1();
        }
        ...
        var this_2 = this, station, svg_station_name_box, svg_station_name;
        for (var i = 0; i < map_data.stations.length; i++) {
            _loop_2();
        }
        var zoomer_svg_group = paper_root.group();
        this.zoomer_svg_group = zoomer_svg_group;
        var zoomer_rect_attr = { fill: 'white', stroke: 'black', strokeWidth: '2px' };
        var zoomer_text_attr = { textAnchor: "middle", dominantBaseline: "middle" };
        var zoom_button_handler = function () {
            _this.update_scale(_this.px_per_minute * 2);
            _this.update_map();
        };
        var unzoom_button_handler = function () {
            _this.update_scale(_this.px_per_minute * 0.5);
            _this.update_map();
        };
        var large_botton_handler = function () {
            _this.update_station_name_size(_this.station_name_size + 5);
            _this.update_map();
        };
        var small_button_handler = function () {
            _this.update_station_name_size(Math.max(_this.station_name_size - 5, 5));
            _this.update_map();
        };
    ...
    MapVisualizer.prototype.set_center_station = function (center_station_id) {
        this.center_station_id = center_station_id;
        this.station_cost = this.calc_time.calcTime(this.center_station_id);
        this.station_pos_update_needed = true;
    };
    MapVisualizer.prototype.update_station_name_size = function (size) {
        this.station_name_size = size;
        for (var i = 0; i < map_data.stations.length; i++) {
            var svg_station_name = this.station_id_to_svg_obj[i];
            var svg_station_name_box = this.station_id_to_svg_box_obj[i];
            var svg_station_name_minibox = this.station_id_to_svg_minibox_obj[i];
    ...
    MapVisualizer.prototype.calc_station_pos = function () {
        var _this = this;
        var positions = {};
        var center_station = this.map_data.stations[this.center_station_id];
        var existing_boxes = [];
        ...
        var _loop_3 = function (i) {
            ...
            var bbox_size = this_3.station_id_to_bbox_size[station_id];
            var x = 0;
            var y = 0;
            var hide = false;
            if (station_id != this_3.center_station_id) {
                var longi_diff = station.longitude - center_station.longitude;
                var lati_diff = station.latitude - center_station.latitude;
                var angle = Math.atan2(-lati_diff, longi_diff);
                var cost = this_3.station_cost[station_id];
                x = this_3.px_per_minute * cost * Math.cos(angle);
                y = this_3.px_per_minute * cost * Math.sin(angle);
            }
            var bbox = {
                left: x - bbox_size.width / 2, right: x + bbox_size.width / 2,
                top: y - bbox_size.height / 2, bottom: y + bbox_size.height / 2
            };
            hide = existing_boxes.some(function (ebb) {
        ...
        var this_3 = this;
        for (var i = 0; i < station_priority.length; i++) {
            _loop_3(i);
        }
        return positions;
    };
